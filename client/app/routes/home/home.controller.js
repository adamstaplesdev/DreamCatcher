...
angular.module('dreamCatcherApp')
	.controller('HomeCtrl', ['$scope', 'navchain', '$timeout', 'goalFactory', '$q', function ($scope, navchain, $timeout, goalFactory, $q) {
		...
		for (var i = 0; i < dreams.length; i++) {
			promises.push(goalFactory.getGoals(dreams[i]._id, 'dream'));
		}

		$q.all(promises).then(function(subgoalsArray) {
			//calculate the progress of each of the dreams by calculating the progress of all of its children
			for (var i = 0; i < dreams.length; i++) {
				var progress = 0;
				var maxProgress = 0;
				for (var j = 0; j < subgoalsArray[i].length; j++) {
					...
					if (!subgoalsArray[i] || !subgoalsArray[i][j] || ! subgoalsArray[i][j].progress)
						...
						progress += subgoalsArray[i][j].progress;
					maxProgress += subgoalsArray[i][j].amount;
				}
